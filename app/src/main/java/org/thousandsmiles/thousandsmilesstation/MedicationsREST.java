...
public class MedicationsREST extends RESTful {
    ...
    private class GetMedicationsResponseListener implements Response.Listener<JSONArray> {
        ...
        public void onResponse(JSONArray response) {
            synchronized (m_lock) {
                SessionSingleton sess = SessionSingleton.getInstance();
                setStatus(200);
                sess.setMedicationsList(response);
                m_lock.notify();
            }
        }
    }
    ...
    public MedicationsREST(Context context) {
        setContext(context);
    }

    public Object getMedicationsList() {
