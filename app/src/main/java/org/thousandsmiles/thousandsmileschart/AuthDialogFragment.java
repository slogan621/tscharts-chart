...
public class AuthDialogFragment extends DialogFragment {
    ...
    public enum AuthDialogType {
        AUTH_DIALOG_INVALID,
        AUTH_DIALOG_CREATE_ACCOUNT,
        AUTH_DIALOG_CHANGE_PASSWORD,
        AUTH_DIALOG_CHANGE_PIN
    };

    private AuthDialogType m_type = AuthDialogType.AUTH_DIALOG_INVALID;

    private boolean validateFields(boolean submit) {
        boolean ret = true;
        AccountFieldValidator validator = new AccountFieldValidator();

        // hide all the validation checkboxes, code below will show if checks pass
        ...
        if (m_type == AuthDialogType.AUTH_DIALOG_CREATE_ACCOUNT) {
            ...
            if (submit && st1.length() == 0) {
                m_last.setError(getString(R.string.error_field_required));
                ret = false;
            }
        }
        ...
        if (m_type == AuthDialogType.AUTH_DIALOG_CHANGE_PASSWORD || m_type == AuthDialogType.AUTH_DIALOG_CREATE_ACCOUNT) {
            ...
            if (!validator.isValidPassword(st1) || !validator.isValidPassword(st2)) {
                // hide checkbox next to password 2
                v.setVisibility(View.GONE);
                if (submit) {
                    getActivity().runOnUiThread(new Runnable() {
                        public void run() {
            ...
            if (st1.length() > 0 && st1.equals(st2)) {
                v.setVisibility(View.VISIBLE);
            } else {
                // hide checkbox next to password 2
                v.setVisibility(View.GONE);
        ...
        if (m_type == AuthDialogType.AUTH_DIALOG_CHANGE_PIN || m_type == AuthDialogType.AUTH_DIALOG_CREATE_ACCOUNT) {
    ...
    public void onResume() {
        ...
        // hide all the validation checkboxes
        ...
        if (m_type == AuthDialogType.AUTH_DIALOG_CREATE_ACCOUNT) {
            m_dialog.findViewById(R.id.create_account_container_name).setVisibility(View.VISIBLE);
        } else {
            m_dialog.findViewById(R.id.create_account_container_name).setVisibility(View.GONE);
        }
        ...
        if (m_type == AuthDialogType.AUTH_DIALOG_CREATE_ACCOUNT || m_type == AuthDialogType.AUTH_DIALOG_CHANGE_PASSWORD) {

            m_dialog.findViewById(R.id.create_account_container_password).setVisibility(View.VISIBLE);
            m_dialog.findViewById(R.id.create_account_container_password_again).setVisibility(View.VISIBLE);
            ...
            m_dialog.findViewById(R.id.create_account_container_password).setVisibility(View.GONE);
            m_dialog.findViewById(R.id.create_account_container_password_again).setVisibility(View.GONE);
        }

        if (m_type == AuthDialogType.AUTH_DIALOG_CREATE_ACCOUNT || m_type == AuthDialogType.AUTH_DIALOG_CHANGE_PIN) {

            m_dialog.findViewById(R.id.create_account_container_pin).setVisibility(View.VISIBLE);
            m_dialog.findViewById(R.id.create_account_container_pin_again).setVisibility(View.VISIBLE);
            ...
            m_dialog.findViewById(R.id.create_account_container_pin).setVisibility(View.GONE);
            m_dialog.findViewById(R.id.create_account_container_pin_again).setVisibility(View.GONE);
        }
    }
    ...
    private void updatePassword(final Dialog dialog, final String username, final String password) {
        new Thread(new Runnable() {
            public void run() {
                final LoginREST x = new LoginREST(getContext());
                final Object lock = x.updatePassword(username, password);
                        ...
                        if (x.getStatus() == 200) {
                            getActivity().runOnUiThread(new Runnable() {
                                public void run() {
                                    ...
                                    AlertDialog alert = builder.create();
                                    alert.show();
                                    Toast.makeText(getContext(), R.string.msg_password_updated_successfully, Toast.LENGTH_LONG).show();
                                    EditText tx = getActivity().findViewById(R.id.email);
                                    tx.setText(m_email.getText());
                                }
                            });
    ...
    private void updatePIN(final Dialog dialog, final String username, final String pin) {
        new Thread(new Runnable() {
            public void run() {
                final LoginREST x = new LoginREST(getContext());
                final Object lock = x.updatePIN(username, pin);
                        ...
                        if (x.getStatus() == 200) {
                            getActivity().runOnUiThread(new Runnable() {
                                public void run() {
                                    ...
                                    AlertDialog alert = builder.create();
                                    alert.show();
                                    Toast.makeText(getContext(), R.string.msg_pin_updated_successfully, Toast.LENGTH_LONG).show();
                                    EditText tx = getActivity().findViewById(R.id.email);
                                    tx.setText(m_email.getText());
                                }
                            });
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            m_type = (AuthDialogType) bundle.get("type");
        }
        ...
        switch(m_type) {
            case AUTH_DIALOG_CHANGE_PASSWORD:
                title = R.string.action_reset_password;
                break;
            case AUTH_DIALOG_CHANGE_PIN:
                title = R.string.action_reset_pin;
                break;
            case AUTH_DIALOG_CREATE_ACCOUNT:
                title = R.string.action_create_account;
                break;
            default:
                Toast.makeText(getContext(), R.string.error_internal_error, Toast.LENGTH_LONG).show();
                return null;
