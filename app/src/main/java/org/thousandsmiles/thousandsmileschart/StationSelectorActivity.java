...
public class StationSelectorActivity extends AppCompatActivity {
                ...
                alertDialogBuilder.setPositiveButton(R.string.button_yes,
                    new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface arg0, int arg1) {
                            m_sess.setStationStationId(id);

                            Intent intent = new Intent(m_activity, PatientSelectorActivity.class);
    ...
    private void LayoutStationGrid() {
        ...
        for (int count = 0; count < numStations; count++) {
            ...
            JSONObject cs = m_sess.getStationData(count);
            if (cs == null) {
                continue;
            }
            String name;
    ...
    protected void onResume() {
        ...
        final Thread thread = new Thread() {
            ...
            if (status == 200) {
                /*
                if (m_sess.updateStationData() == false) {
                    StationSelectorActivity.this.runOnUiThread(new Runnable() {
                        public void run() {
                            Toast.makeText(getApplicationContext(), R.string.error_unable_to_get_station_data, Toast.LENGTH_LONG).show();
                        }
                 ...
                 */
                    ...
                    //m_sess.initCategoryNameToUnvisitedSelectorMap();      // for unvisited routing slip entries
                    StationSelectorActivity.this.runOnUiThread(new Runnable() {
                        public void run() {
                            LayoutStationGrid();
                        }
